...
app.post('/addtodo', expressJWT({ secret: app.get('superSecret') }), function(req, res) {
    User.findOne({ username: req.user._doc.username }, function(err: any, user: any) {
        if (err) throw err;
        var todo = new Todo({
...
app.post('/updatetodo', expressJWT({ secret: app.get('superSecret') }), function(req, res) {
    Todo.findById(req.body.todo._id, function(err: any, todo: any) {
        if (err) throw err;
